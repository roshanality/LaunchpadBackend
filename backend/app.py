...
def get_service_detail(service_id):
    ...
    try:
        ...
        # Include timeline items in the same response so detail page has them
        try:
            cursor.execute(
                '''SELECT id, title, description, date, image, status, category, sort_order
                   FROM service_timeline_items WHERE service_id = ? ORDER BY sort_order ASC, date ASC, id ASC''',
                (service_id,),
            )
        ...
        # Include reviews & past work for the service
        try:
            cursor.execute(
                '''SELECT id, author_name, content, rating, created_at
                   FROM service_reviews WHERE service_id = ? ORDER BY created_at DESC''',
                (service_id,),
            )
...
@app.route('/api/launchpad/services/<int:service_id>/timeline', methods=['GET'])
def get_service_timeline(service_id):
    """Get timeline items for a service (public)."""
    db_path = get_db_path()
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    try:
...
@app.route('/api/admin/services/<int:service_id>/timeline', methods=['GET'])
@jwt_required()
def admin_get_service_timeline(service_id):
    user_id = get_user_id_from_jwt()
    if not is_admin(user_id):
        return jsonify({'error': 'Admin access required'}), 403
    db_path = get_db_path()
    ...
    try:
        ...
        items = [
            {
                'id': r[0], 'title': r[1], 'description': r[2], 'date': r[3] or '',
                'image': r[4] or '', 'status': r[5] or 'upcoming', 'category': r[6] or '', 'sort_order': r[7] or 0,
            }
            for r in rows
        ]
        return jsonify(items), 200
...
@app.route('/api/admin/services/<int:service_id>/timeline', methods=['POST'])
@jwt_required()
def admin_create_timeline_item(service_id):
    user_id = get_user_id_from_jwt()
    if not is_admin(user_id):
        return jsonify({'error': 'Admin access required'}), 403
    ...
    try:
        ...
        conn.commit()
        return jsonify({'message': 'Timeline item created', 'id': item_id}), 201
    except Exception as e:
        conn.rollback()
        return jsonify({'error': str(e)}), 500
    finally:
...
@jwt_required()
def admin_update_timeline_item(item_id):
    user_id = get_user_id_from_jwt()
    if not is_admin(user_id):
        return jsonify({'error': 'Admin access required'}), 403
    data = request.get_json() or {}
    ...
    try:
        ...
        cursor.execute(
            f'UPDATE service_timeline_items SET {", ".join(updates)} WHERE id = ?',
            params,
        )
        conn.commit()
        return jsonify({'message': 'Timeline item updated'}), 200
    except Exception as e:
        conn.rollback()
        return jsonify({'error': str(e)}), 500
    finally:
...
def admin_delete_timeline_item(item_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('DELETE FROM service_timeline_items WHERE id = ?', (item_id,))
        conn.commit()
...
# Get current user's service requests (for Ask Services timeline)
@app.route('/api/launchpad/my-requests', methods=['GET'])
@jwt_required()
def get_my_service_requests():
    ...
    try:
        cursor.execute('''
            SELECT sr.id, sr.project_type, sr.description, sr.budget_range, sr.status, sr.created_at,
                   sr.admin_notes
            FROM service_requests sr
            WHERE sr.user_id = ?
            ORDER BY sr.created_at DESC
...
def _require_student():
    """Get user_id from JWT and require role=student. Returns (user_id, error_response)."""
    user_id = get_user_id_from_jwt()
    db_path = get_db_path()
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    ...
    if not row or row[0] != 'student':
        return None, (jsonify({'error': 'Only students can access this'}), 403)
    return user_id, None


@app.route('/api/launchpad/student-profile', methods=['GET'])
@jwt_required()
def get_student_service_profile():
    ...
    try:
        cursor.execute('''
            SELECT resume_url, skills, experience, education, linkedin_url, portfolio_url, other_info, created_at, updated_at
            FROM student_service_profiles WHERE user_id = ?
        ''', (user_id,))
        row = cursor.fetchone()
        if not row:
            return jsonify({}), 200
...
@app.route('/api/launchpad/student-profile', methods=['POST'])
@jwt_required()
def upsert_student_service_profile():
    user_id, err = _require_student()
    if err:
        return err[0], err[1]
    data = request.get_json() or {}
    ...
    try:
        ...
        cursor.execute('SELECT id FROM student_service_profiles WHERE user_id = ?', (user_id,))
...
@app.route('/api/launchpad/my-allotments', methods=['GET'])
@jwt_required()
def get_my_allotments():
    ...
    try:
        cursor.execute('''
            SELECT sa.id, sa.service_id, sa.status, sa.created_at, sa.agreed_at, s.title as service_title
...
@app.route('/api/launchpad/allotments', methods=['POST'])
@jwt_required()
def create_allotment():
    """Admin or provider: select a student for a service (creates pending_agreement allotment)."""
    user_id = get_user_id_from_jwt()
    if not is_admin(user_id):
        return jsonify({'error': 'Admin access required'}), 403
    data = request.get_json() or {}
    ...
    if not service_id or not student_id:
        return jsonify({'error': 'service_id and student_id required'}), 400
    db_path = get_db_path()
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    try:
        ...
        cursor.execute('SELECT id FROM service_allotments WHERE service_id = ? AND student_id = ?', (service_id, student_id))
        if cursor.fetchone():
            return jsonify({'error': 'Student already allotted to this service'}), 400
        cursor.execute(
            'INSERT INTO service_allotments (service_id, student_id, status) VALUES (?, ?, ?)',
            (service_id, student_id, 'pending_agreement')
        )
        conn.commit()
        return jsonify({'message': 'Student selected for service. They can agree from their Service Profile.'}), 201
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    finally:
        conn.close()
...
@app.route('/api/launchpad/allotments/<int:allotment_id>/agree', methods=['POST'])
@jwt_required()
def agree_allotment(allotment_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('SELECT id, status FROM service_allotments WHERE id = ? AND student_id = ?', (allotment_id, user_id))
        row = cursor.fetchone()
        if not row:
            return jsonify({'error': 'Allotment not found'}), 404
        if row[1] != 'pending_agreement':
            return jsonify({'error': 'Already responded to this allotment'}), 400
        now = datetime.now().isoformat()
        cursor.execute('UPDATE service_allotments SET status = ?, agreed_at = ? WHERE id = ?', ('agreed', now, allotment_id))
        conn.commit()
        return jsonify({'message': 'You have agreed to this service. The team will reach out to you shortly.'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    finally:
        conn.close()
...
# Student verification endpoints (roll number + ID card upload)
@app.route('/api/students/verification', methods=['GET'])
@jwt_required()
def get_student_verification():
    user_id = get_user_id_from_jwt()
    ...
    try:
        cursor.execute('SELECT roll_number, id_card_image FROM users WHERE id = ?', (user_id,))
        row = cursor.fetchone()
        if not row:
...
@app.route('/api/students/verification', methods=['POST'])
@jwt_required()
def update_student_verification():
    ...
    try:
        ...
        if request.content_type and 'multipart/form-data' in request.content_type:
            ...
            if 'id_card' in request.files:
                ...
                if file.filename:
                    ...
                    file.save(os.path.join(app.config['UPLOAD_FOLDER'], id_card_filename))
        else:
            data = request.get_json() or {}
        ...
        if id_card_filename:
            ...
            params.append(id_card_filename)
...
@app.route('/api/admin/students/verification', methods=['GET'])
@jwt_required()
def get_admin_student_verifications():
    """List all students with verification data (roll_number or id_card_image) for admin panel."""
    user_id = get_user_id_from_jwt()
    if not is_admin(user_id):
        return jsonify({'error': 'Admin access required'}), 403
    db_path = get_db_path()
    ...
    try:
        ...
        for row in rows:
            result.append({
                ...
                'email': row[2],
                'role': row[3],
                'roll_number': row[4] or '',
                'id_card_image': row[5] or '',
...
def get_available_users():
    try:
        ...
        cursor = conn.cursor()
        # Students can only message other students; alumni can message anyone
        cursor.execute('SELECT role FROM users WHERE id = ?', (user_id,))
        role_row = cursor.fetchone()
