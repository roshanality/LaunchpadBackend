...
def register():
    ...
    if data["role"] == "alumni":
        if not data.get("graduation_year") or not data.get("department"):
            return (
                jsonify(
                    {"error": "Graduation year and department are required for alumni"}
                ),
                400,
            )
...
def login():
    ...
    try:
        ...
        if user["role"] in ("mentor", "investor", "founder", "alumni") and not user.get(
            "is_approved"
        ):
            return (
                jsonify(
...
def verify_email():
    ...
    return (
        jsonify(
            {"message": "Email verification is not yet configured on this server."}
        ),
        501,
    )
